...
Plugin.prototype = {
  ...
  * @param {LabelGraphics} options.billboard - see {@link module:Graphics#getBillboardGraphics|getBillboardGraphics}
  ...
  buildPathRoaming: function (options) {
    if (this._viewer && options && options.paths) {
      ...
      for (var i = 0; i < _paths.length; i++) {
        var cartesian = Cesium.Cartesian3.fromDegrees(Number(_paths[i].lng), Number(_paths[i].lat), Number(_paths[i].alt));
    ...
    this._viewer.clock.onTick.addEventListener(() => {
      if (this._viewer.clock.shouldAnimate) {
        ...
        if (position && orientation) {
          //计算相机的四维矩阵
          let transform = Cesium.Matrix4.fromRotationTranslation(Cesium.Matrix3.fromQuaternion(orientation), position);
  ...
  _createSampledPosition(positions) {
    ...
    // 结束时间（长度*60）计算总时长
    this.stopTime = Cesium.JulianDate.addSeconds(this.startTime, positions.length * 60, new Cesium.JulianDate());
    // 设置时钟开始时间
    ...
    for (let j = 0; j < positions.length; j++) {
      let time = Cesium.JulianDate.addSeconds(this.startTime, j * 60, new Cesium.JulianDate());
  ...
  _sampleOrientation() {
    ...
    return new Cesium.CallbackProperty((currentTime) => {
      ...
      if (position && positionB) {
        positionA = position;
        //向量AB差获得朝向
        const vector2 = Cesium.Cartesian3.subtract(positionB, positionA, new Cesium.Cartesian3());
        //归一化获得单位向量（方向）
