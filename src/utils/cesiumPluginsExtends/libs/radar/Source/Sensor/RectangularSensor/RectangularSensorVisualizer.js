...
import { initRectangular } from './RectangularSensorPrimitive'
// import { removePrimitive } from '../../Core/remove-primitive'

function initVisualize(cesiumObj) {
  Cesium = cesiumObj
  ...
  var removePrimitive = function (entity, hash, primitives) {
  ...
  RectangularSensorVisualizer.prototype.update = function (time) {
    ...
    for (var i = 0, len = entities.length; i < len; i++) {
      ...
      if (!defined(primitive)) {
        primitive = new initRectangular(Cesium) // new RectangularSensorPrimitive()
        primitive.id = entity
